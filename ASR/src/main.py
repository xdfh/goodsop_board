...
app = FastAPI(
    title="ASR API",
    description=f"一个使用 TFLite 模型的语音转文字 API (设备: {settings.ASR_DEVICE}, 精度: {settings.ASR_MODEL_PRECISION})",
    version="1.0.0",
)
...
async def transcribe_audio(file: UploadFile = File(...)):
    ...
    try:
        ...
        if num_samples <= CHUNK_SAMPLES:
            audio_data_int8 = (audio_data * 127).astype(np.int8)
            audio_data_batched = np.expand_dims(audio_data_int8, axis=0)
            ...
            for i in range(0, num_samples, CHUNK_SAMPLES):
                ...
                chunk_int8 = (chunk * 127).astype(np.int8)
                chunk_batched = np.expand_dims(chunk_int8, axis=0)
